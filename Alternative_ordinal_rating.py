...
def create_design():
    ...
    trials = list(itertools.combinations(range(len(r)), 2)) #Makes combinations (1,2,3-1,2,4-1,2,5-1,2,6-1,3,4-1,3,5-1,3,6-etc...)
    ...
    # s3 = []
    for t in trials:
        t1, t2 = shuffle_triad(t) #t1, t2, t3 = shuffle_triad(t)
        
        s1.append(t1)
        s2.append(t2)
        # s3.append(t3)
    
    new_data = {'S1': s1, 'S2': s2}
...
class Experiment(window.Window):
    ...
    def __init__(self, *args, **kwargs):
        ...
        rfls = glob.glob('./data/%s_ordinal_%s_%s*.csv' % (obsname,selectedImage,selectedProcedure))
        ...
        self.resultsfile = './data/%s_ordinal_%s_%s_%d.csv' % (obsname,selectedImage,selectedProcedure, nb)
    ...
    def on_draw(self):
        ...
        # go through the trials 
            ...
            if (presentation_time is None) or (self.present_stim):
                ...
                self.image1.blit(int(self.width*0.25), int(self.height*0.5))
                self.image2.blit(int(self.width*0.75), int(self.height*0.5))
    ...
    def savetrial(self, resp, resptime):
        ...
        s1 = self.design['S1'][self.currenttrial] #Das Vergleichsbild #Linkes
        s2 = self.design['S2'][self.currenttrial] #Linkes Bild #Rechtes
